<script lang='ts'>
  ...
  function openModal(speaker: SpeakersDocument, from: 'session' | 'talk' | null = null) {
    if (from === 'talk') {
...
{#if selectedSpeaker}
  ...
  <!-- Speaker Modal -->
  <div transition:fly={{x:"-100%"}} class="fixed inset-0 z-[70] flex items-center justify-center p-4 pointer-events-none">
    ...
    >
      <div class="sticky top-0 bg-white border-b border-black/10 px-6 py-4 flex items-center justify-end">
        <button 
          ...
          aria-label={previousModal === 'session' ? 'Back to session' : 'Close modal'}
        >
          ...
          <div class="flex-1 flex flex-col gap-4">
            <div>
              ...
              {#if selectedSpeaker.data.organization}
                <h6 class="text-black/60">{selectedSpeaker.data.organization}</h6>
              {/if}
            </div>
...
{#if selectedSession && !selectedTalk && !selectedSpeaker}
  ...
  <div transition:fly={{y: "100%"}} class="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
    ...
    <div 
      ...
      aria-modal="true"
    >
      <div class="sticky top-0 bg-white border-b border-black/10 px-3 md:px-6 py-4 flex items-center justify-between">
        <div>
                ...
                <div class="flex flex-col md:flex-row gap-6">
                  <div class="flex-1">
                    <div class="flex items-start justify-between gap-4 mb-3">
                      ...
                      >
                        <h5 class="text-black group-hover:text-primary transition cursor-pointer">{talk.data.name}</h5>
                      </button>
...
{#if selectedTalk && !selectedSpeaker}
  ...
  <!-- Talk Modal -->
  <div transition:fly={{x: "100%"}} class="fixed inset-0 z-[60] flex items-center justify-center p-4 pointer-events-none">
    ...
    <div 
      ...
      aria-modal="true"
    >
      <div class="sticky top-0 bg-white border-b border-black/10 px-3 md:px-8 py-4 flex items-start justify-between">
        <div class="flex-1 pr-4">
          <h4 class="text-primary">{selectedTalk.data.name}</h4>
          {#if selectedTalk.data.subtitle}
            <p class="text-black/70 text-sm mt-1">{selectedTalk.data.subtitle}</p>
          {/if}
        ...
        {/if}
          ...
          {#if speaker}
            <div class="mt-6 pt-6 border-t border-black/10">
              ...
              >
                {#if speaker.data.headshot}
                  <div class="w-16 h-16 rounded-full overflow-hidden bg-light/40">
                    <PrismicImage 
                      field={speaker.data.headshot} 
                      class="w-full h-full object-cover" 
                    />
                ...
                {/if}
                <div class="text-left">
                  <div class="font-medium text-black group-hover:text-primary transition">
                    {speaker.data.name}
                  </div>
...
<section id="qr" class="w-screen to-white/20 from-light/80 from-25% bg-radial-[at_50%_75%] relative add-noise">
...
<section id="schedule" class="w-screen to-white/20 from-light/80 from-25% bg-radial-[at_50%_25%] pt-24 pb-6 relative add-noise overflow-y-visible">
...
<section id="featured-presenters" class="w-screen to-white/20 from-light/80 from-25% bg-radial-[at_50%_75%] pt-24 pb-6 relative add-noise overflow-y-visible">
  <ContentWidth class="flex flex-col gap-12">
...
<section id="map" class="w-screen to-white/20 from-light/80 from-25% bg-radial-[at_50%_25%] pt-24 pb-6 relative add-noise overflow-y-visible">
	 <ContentWidth class="flex flex-col gap-12"> 
...
<section id="sponsors" class="w-screen to-white/20 from-light/80 from-25% bg-radial-[at_50%_75%] pt-24 pb-6 relative add-noise overflow-y-visible">
	 <ContentWidth class=""> 
...
<section id="all-presenters" class="w-screen to-white/20 from-light/80 from-25% bg-radial-[at_50%_25%] pt-24 pb-6 relative add-noise overflow-y-visible">
  <ContentWidth class="flex flex-col gap-12">
